...
def parse_show_mac_address_table(raw_result):
    ...
    mac_entry = (
        ...
        r'(?P<vlan_id>[0-9]+)\s*'
        r'(?P<from>[- a-zA-Z]+)\s+(?P<port>([lag]+)?\d+[-.]?\d*)'
    )
...
def parse_show_ip_interface(raw_result):
    ...
    if "lag" in raw_result:
        return parse_show_ip_interface_lag(raw_result)
...
def parse_show_ipv6_interface(raw_result):
    ...
    if "lag" in raw_result:
        return parse_show_ipv6_interface_lag(raw_result)
...
def parse_access_list_section(raw_result, result):
    ...
    for line in raw_result.splitlines():
        ...
        # print("before search (" + parsing_state + ") <" + line + ">")
